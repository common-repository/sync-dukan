<?php
/*
 * Plugin Name: Sync Dukan
 * Version: 1.0
 * Description: Syncs dukan.pk!
 * Author: Inspurate <info@inspurate.com>
 * Author URI: https://www.inspurate.com/about
 * Plugin URI:  https://www.inspurate.com/plugins/sync-dukan
 */
...
//                $this->getDukanOrders();
    ...
    ";
        ...
        private function getDukanProductsApiCall($id = 0, $from_date)
        {
...
//
        ...
        public function getDukanProducts($product_id = 0, $from_date = 0)
        {
            $d_products = $this->getDukanProductsApiCall($product_id, $from_date);
            ...
            if(count($d_products) > 0) {
                foreach($d_products as $d_product) {
                    ...
                    $post_id = wc_get_product_id_by_sku('dukan_' . $d_product->product_id);
...
//                            if(!empty($d_product->gallery_images)) {
//Already Commented.                                $this->setPostGalleryImages($post_id, $d_product->product_picture);
//                            }
...
//                            break;
                ...
                if($from_date == 0) {
                    ...
                    echo esc_html( __('All the products were imported successfully.', 'text_domain' ) );
                    exit;
                }
            }
            return;
        ...
        public function getDukanOrders($from_date = 0)
        {
            ...
            if($data->success == 'Y') {
                ...
                //echo 'Number of orders: ' . count($d_orders);
                if(count($d_orders) > 0) {
                    foreach($d_orders as $d_order) {
                        ...
                        foreach ($d_order->order_items as $item) {
                            $order_item_id = wc_get_product_id_by_sku('dukan_'.$item->product_id);
                            if(!$order_item_id) {
                ...
                if($from_date == 0) {
                    echo esc_html( __('<h2>All the orders were imported successfully.', 'text_domain' ) );
                    exit;
                }
            }
 ...
 installed) WooCommerce before proceeding", "text_domain" ) );
            ...
            echo esc_html_e( __("Connection Code: ". get_option('dukan_access_code'), "socialize"));

            include_once 'views/status.php';

            if(get_option('dukan_cron')) {
        ...
        private function eligibility()
        {
            if (get_option('dukan_connection') != 1) {
                ...
                echo esc_html( __('<div class="success notice">Your dukan access code: ' . get_option('dukan_access_code') . '</div>', 'text_domain' ) );
